...
export container_name=jm-redis-dev;
export project_name=jm-redis;
export image_name=ubuntu-redis;
export ip=${jm_redis_ip};
...
mbt_rewrite;
devCreate() {
	if ! docker run -d --name $container_name --net aegis-bridge --ip $ip -p 6012:6379 $image_name > /dev/null; then
